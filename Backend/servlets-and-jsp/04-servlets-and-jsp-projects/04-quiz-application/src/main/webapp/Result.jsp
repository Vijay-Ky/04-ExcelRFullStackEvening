...
<center>
<input  onclick="location.href='com.vp.main.controller.LogoutStudent'" type="button" value="Exit" class="btn btn-outline-danger btn-lg">
</center>
...
<%
    ...
    for (int i = 0; i < size; i++) {
        dbans[i] = allQuestions.get(i).getAnswer();
        userans[i] = request.getParameter("q" + i);
    }
    ...
    for (int k = 0; k < size; k++) {
        if (userans[k] != null && userans[k].equalsIgnoreCase(dbans[k])) {
            correctanswer++;
        } else if (userans[k] == null || userans[k].equals("")) {
            unattempted++;
        } else {
            wronganswer++;
        }
